...
Sully.ai supports transcription in multiple languages using [BCP47 language tags](https://en.wikipedia.org/wiki/IETF_language_tag). See [Supported Languages](/api-reference/audio-transcriptions/languages) for the full list of **89** accepted locale codes.
...
<Note>
For most locale codes, audio in other languages is filtered out. Base tags such as `es`, `fr`, and `de` route to multilingual code-switching instead — see [automatic multilingual routing](/api-reference/audio-transcriptions/languages#automatic-multilingual-routing). Use `multi` on streaming when speakers switch languages freely.
</Note>
